...
class DocumentosNumeroComprobantesController extends ControladorBase{
	...
	public function index(){
		...
		if (isset(  $_SESSION['usuario_usuario']) )
		{
			...
			if (!empty($resultPer))
			{
				...
				$resultPol = $documentos_legal->getCondiciones("comprobantes.numero_comprobantes, comprobantes.id_comprobantes", "public.documentos_legal, public.comprobantes", "comprobantes.id_comprobantes = documentos_legal.id_comprobantes ", "comprobantes.numero_comprobantes");
				...
				
				{
					...
					if ($_numero_comprobantes > 0)
					{
						...
						$where_4 = " AND comprobantes.numero_comprobantes = '$_numero_comprobantes' ";
					}	
			...
			if ($resultSet != "")
			{
				...
				$this->view("DocumentosNumeroComprobantes",array(
					...
					if($action == 'ajax')
					{
						...
						if ($cantidadResult>0)
						{
							...
							foreach ($resultSet as $res)
							{
								...
								$html.='<td style="color:#000000;font-size:80%;">'.$res->numero_comprobantes.'</td>';
								$html.='<td style="color:#000000;font-size:80%;">'.$res->paginas_documentos_legal.'</td>';
								$html.='<td style="color:#000000;font-size:80%;">'.$res->nombre_referencia.'</td>';
	...
	public function AutocompleteNumeroCredito(){
		...
		$numero_comprobantes = strtoupper($_GET['term']);
		$_numero_comprobantes = array();
		
		$documentos_legal = new DocumentosLegalModel();
	
	
		//Tipo de Documento
		$resultPol = $documentos_legal->getCondiciones("comprobantes.id_comprobantes, comprobantes.numero_comprobantes",
				"documentos_legal, public.comprobantes", 
				"comprobantes.id_comprobantes = documentos_legal.id_comprobantes AND comprobantes.numero_comprobantes LIKE '$numero_comprobantes%' ",
		...
		if(!empty($resultPol)){
			...
			foreach ($resultPol as $res){
				...
				$_numero_credito[] = array('id' => $res->numero_comprobantes, 'value' => $res->numero_comprobantes);
			}
