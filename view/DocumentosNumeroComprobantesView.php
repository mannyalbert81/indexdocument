...
<?php require_once 'config/global.php';?> 
	...
	function load_DocumentosNumeroCred(pagina){
		...
		//iniciar variables
		 ...
		 var doc_numero_cred=$("#numero_comprobantes").val();
		 ...
		 var doc_year=$("#year").val();
		  ...
		  var con_datos={
				  ...
				  numero_comprobantes:doc_numero_cred,
		...
		$.ajax({
			url:"<?php echo $helper->url("DocumentosNumeroComprobantes","buscar");?>",
            type : "POST",
            async: true,			
			data: con_datos,
	...
	$("#txt_numero_comprobantes").autocomplete({
		source: "<?php echo $helper->url("DocumentosNumeroComprobantes","AutocompleteNumeroCredito"); ?>",
		minLength: 1,
		select: function( event, data ) 
			{
			 ...
			 $("#txt_numero_comprobantes").val(valor);
			 $("#numero_comprobantes").val(identificador);
			 
			}
	 });
		
	$("#txt_numero_comprobantes").focusout(function(){

		if($("#txt_numero_comprobantes").val()==''||$("#txt_numero_comprobantes").val()==null)
		{
			 $("#txt_numero_comprobantes").val('');
			 $("#numero_comprobantes").val(0);
    ...
    <body style="background-color: #F6FADE">
       ...
       <?php
		   ...
		   if($_SERVER['REQUEST_METHOD']=='POST' )
		   {
		      ...
		      $sel_year = $_POST['year'];
		      $sel_numero_comprobantes = $_POST['numero_comprobantes'];
      ...
      <form id="formularioPrincipal" action="<?php echo $helper->url("DocumentosNumeroComprobantes","index"); ?>" method="post" class="form-horizontal">
        
     
      <div class="panel panel-default">

		  <div class="panel-heading"> <strong style="color:#ec971f;"> BUSQUESA POR NUMERO DE COMPROBANTES </strong>  </div>
      ...
      <div class="table-responsive">
       	...
       	<table class="col-sm-12">     	
            ...
            <tr>
	    		...
	    		<th class="col-sm-1">Numero Comprobantes</th>
            ...
            <tr>
				...
				<td>	       		
		          <select name="year" id="year"  class="form-control">
		   		...
		   		<td>
		   		 <?php 
		   		 $_numero_comprobantes=0;
		   		if($resultEdit !="" && !empty($resultPol))
		   		{
		   			$numero_comprobantes=$resultEdit[0]->numero_comprobantes;
		   			
		   			foreach ($resultPol as $resPol)
		   			{
		   				if($numero_comprobantes==$resPol->numero_comprobantes)
		   				{
		   					$_numero_comprobantes=$resPol->numero_comprobantes;
		   		...
		   		?>
		   		   ...
		   		   <?php if ($resultEdit !="" ) { foreach($resultEdit as $resEdit) {?>
						...
						<input type="text" class="form-control" id="txt_numero_comprobantes_edit" name="txt_numero_comprobantes_edit" value="<?php  echo $_numero_comprobantes; ?>">
                 		<input type="hidden"  id="numero_comprobantes" name="numero_comprobantes" value="<?php echo  $res->numero_comprobantes; ?>">	
					     <?php } } else {?>
					     
					     <input type="text" class="form-control" id="txt_numero_comprobantes" name="txt_numero_comprobantes" value=""  placeholder="Ingrese Numero Comprobante">
                 		 <input type="hidden"  id="numero_comprobantes" name="numero_comprobantes" value="0">									
						 <?php } ?>	  
